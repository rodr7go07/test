...
@php
    $isOpen = collect($routes)->contains(fn($r) => request()->routeIs($r));
@endphp

<li x-data="{ open: {{ $isOpen ? 'true' : 'false' }} }">
    <button @click="open = !open" type="button"
            class="flex items-center w-full p-2 text-base font-normal transition rounded-lg
        {{ $isOpen ? 'bg-gray-200 dark:bg-gray-700' : 'hover:bg-gray-100 dark:hover:bg-gray-700' }}">
        <i class="{{ $icon }} w-5 h-5"></i>
        <span class="flex-1 ml-3 text-left whitespace-nowrap">{{ $label }}</span>
        <svg :class="{ 'rotate-180': open }" class="w-4 h-4 transition-transform ml-auto" fill="none"
             stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M19 9l-7 7-7-7" />
        </svg>
    </button>
    <ul x-show="open" x-transition class="space-y-2 pl-7 mt-1">
        {{ $slot }}
    </ul>
</li>
